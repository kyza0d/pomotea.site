...
export const SpotlightEffect: React.FC<SpotlightEffectProps> = ({
  ...
  void main() {
    ...
    float intensity_dist = arc_distance / max(curve_factor, 0.09);
...
`;
  ...
  useEffect(() => {
    ...
    const render = (now: number) => {
      ...
      velocity.current.x =
        velocity.current.x * velocityDampening +
        deltaX * velocityResponsiveness;
      velocity.current.y =
        velocity.current.y * velocityDampening +
        deltaY * velocityResponsiveness;
      ...
      inertiaOffset.current.x +=
        (velocity.current.x * inertiaStrength - inertiaOffset.current.x) *
        inertiaCatchUp;
      inertiaOffset.current.y +=
        (velocity.current.y * inertiaStrength - inertiaOffset.current.y) *
        inertiaCatchUp;

      // Smooth easing for position transitions
      const smoothingFactor = 0.04;
      animatedPosition.current.x +=
        (currentTarget.x - animatedPosition.current.x) * smoothingFactor;
      animatedPosition.current.y +=
        (currentTarget.y - animatedPosition.current.y) * smoothingFactor;
      ...
      const inertiaOffsetLocation = gl.getUniformLocation(
        program,
        "u_inertiaOffset",
      );
      ...
      const secondarySaturatedColorLocation = gl.getUniformLocation(
        program,
        "u_secondarySaturatedColor",
      );
      const glowColorLocation = gl.getUniformLocation(program, "u_glowColor");

      // Get colors from CSS variables
      const primaryColor = parseCSSColor(
        getCSSVariable("--color-landing-glow"),
      );
      const secondaryColor = parseCSSColor(
        getCSSVariable("--color-landing-glow"),
      );
      const secondarySaturatedColor = parseCSSColor(
        getCSSVariable("--color-landing-glow"),
      );
      ...
      gl.uniform3f(
        secondarySaturatedColorLocation,
        secondarySaturatedColor[0],
        secondarySaturatedColor[1],
        secondarySaturatedColor[2],
      );
